...
//  ********** Library C:\Users\Rob\Natzke Ribbons\01_Particle\02_Move_Particles\ParticleEmitter02 **************
// ********** Code for ParticleEmitter02 **************
function ParticleEmitter02() {

}
ParticleEmitter02.prototype.run = function() {
  var $this = this; // closure support
  this.mouseX = (0);
  this.mouseY = (0);
  this.particleNum = (0);
...
ParticleEmitter02.prototype.onDocumentMouseMove = function(event) {
  this.mouseX = event.get$pageX();
  this.mouseY = event.get$pageY();
}
ParticleEmitter02.prototype.get$onDocumentMouseMove = function() {
  return this.onDocumentMouseMove.bind(this);
}
ParticleEmitter02.prototype.spawnParticle = function() {
  this.particleNum++;
  this.particles.$setindex(("p " + this.particleNum), _map(["x", this.mouseX, "y", this.mouseY, "xSpeed", Math.random() * (4) - (2), "ySpeed", Math.random() * (4) - (2)]));
}
ParticleEmitter02.prototype.moveParticles = function() {
  this.context.clearRect((0), (0), this.canvas.get$width(), this.canvas.get$height());
  var $$list = this.particles.getKeys();
  for (var $$i = $$list.iterator(); $$i.hasNext(); ) {
    var key = $$i.next();
    var p = this.particles.$index(key);
    p.$setindex("x", $add$(p.$index("x"), p.$index("xSpeed")));
    p.$setindex("y", $add$(p.$index("y"), p.$index("ySpeed")));
    this.context.set$fillStyle("rgba( 255, 255, 255, 1 )");
    this.context.beginPath();
    this.context.arc(p.$index("x"), p.$index("y"), (10), (0), (6.283185), false);
    this.context.closePath();
    this.context.fill();
    this.context.set$fillStyle("rgba( 50, 50, 50, 1 )");
    this.context.beginPath();
    this.context.arc(p.$index("x"), p.$index("y"), (8), (0), (6.283185), false);
    this.context.closePath();
    this.context.fill();
  }
}
...
(function(){
  ...
  var v9/*EventTarget*/ = [v2/*AbstractWorker*/,v3/*Node*/,'EventTarget|DOMApplicationCache|DOMWindow|EventSource|MessagePort|Notification|SVGElementInstance|WebSocket|XMLHttpRequest|XMLHttpRequestUpload'].join('|');
  var v10/*HTMLCollection*/ = 'HTMLCollection|HTMLOptionsCollection';
  var v11/*IDBRequest*/ = 'IDBRequest|IDBVersionChangeRequest';
  var v12/*MediaStream*/ = 'MediaStream|LocalMediaStream';
  var v13/*WorkerContext*/ = 'WorkerContext|DedicatedWorkerContext|SharedWorkerContext';
  var table = [
