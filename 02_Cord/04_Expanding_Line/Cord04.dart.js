...
$$.Cord04 = {"":
 ...
 moveParticles$0: function() {
  ...
  if ($.ltB(min, 0)) min = 0;
  if (typeof min !== 'number') return this.moveParticles$0$bailout(1, min, 0);
  var c = $.sub(this.particleNum, 1);
  if (typeof c !== 'number') return this.moveParticles$0$bailout(2, c, min);
  var t2 = this.gravity;
  var t3 = this.airFriction;
  var t4 = this.ground;
  var t5 = this.maxDistance;
  for (; c > min; --c) {
    var p = $.index(this.particles, 'p ' + $.S(c));
    var nb = $.index(this.particles, 'p ' + $.S(c - 1));
    p.set$xSpeed($.add(p.get$xSpeed(), this.wind));
    p.set$ySpeed($.add(p.get$ySpeed(), t2));
    p.set$xSpeed($.mul(p.get$xSpeed(), t3));
    p.set$ySpeed($.mul(p.get$ySpeed(), t3));
    p.set$x($.add(p.get$x(), p.get$xSpeed()));
    p.set$y($.add(p.get$y(), p.get$ySpeed()));
    p.set$radius($.add(p.get$radius(), 0.5));
    if ($.gtB(p.get$y(), t4)) {
      p.set$y(t4);
    ...
    var Distance = $.Math_sqrt($.add($.mul(X, X), $.mul(Y, Y)));
    var Angle = $.Math_atan2(Y, X);
    if ($.gtB(Distance, t5)) {
      var t1 = nb.get$x();
      var t6 = $.Math_cos(Angle);
      if (typeof t6 !== 'number') throw $.iae(t6);
      p.set$x($.add(t1, t5 * t6));
      var t7 = nb.get$y();
      var t8 = $.Math_sin(Angle);
      if (typeof t8 !== 'number') throw $.iae(t8);
      p.set$y($.add(t7, t5 * t8));
    }
  }
  this.context.clearRect$4(0, 0, this.canvas.get$width(), this.canvas.get$height());
  c = $.sub(this.particleNum, 1);
  if (typeof c !== 'number') return this.moveParticles$0$bailout(3, c, min);
  for (; c > min; --c) {
    p = $.index(this.particles, 'p ' + $.S(c));
    nb = $.index(this.particles, 'p ' + $.S(c - 1));
    this.context.beginPath$0();
    t1 = p.get$radius();
    this.context.set$lineWidth(t1);
    t1 = c === $.sub(this.particleNum, 1);
    t2 = this.context;
    if (t1) t2.moveTo$2(p.get$x(), p.get$y());
    else {
      t2.moveTo$2(nb.get$x(), nb.get$y());
      this.context.lineTo$2(p.get$x(), p.get$y());
    }
  ...
  c = $.sub(this.particleNum, 1);
  if (typeof c !== 'number') return this.moveParticles$0$bailout(4, c, min);
  for (; c > min; --c) {
    p = $.index(this.particles, 'p ' + $.S(c));
 ...
 moveParticles$0$bailout: function(state, env0, env1) {
  switch (state) {
    case 1:
      min = env0;
      break;
  ...
  switch (state) {
    ...
    case 2:
      ...
      for (; $.gtB(c, min); c = $.sub(c, 1)) {
        var p = $.index(this.particles, 'p ' + $.S(c));
        var nb = $.index(this.particles, 'p ' + $.S($.sub(c, 1)));
        p.set$xSpeed($.add(p.get$xSpeed(), this.wind));
        p.set$ySpeed($.add(p.get$ySpeed(), t2));
        p.set$xSpeed($.mul(p.get$xSpeed(), t3));
        p.set$ySpeed($.mul(p.get$ySpeed(), t3));
    ...
    case 3:
      ...
      for (; $.gtB(c, min); c = $.sub(c, 1)) {
        p = $.index(this.particles, 'p ' + $.S(c));
        nb = $.index(this.particles, 'p ' + $.S($.sub(c, 1)));
        this.context.beginPath$0();
        t1 = p.get$radius();
        this.context.set$lineWidth(t1);
 ...
 spawnParticle$0: function() {
  var mag = $.mul($.Math_random(), 10);
  var dir = $.mul($.mul($.Math_random(), 3.141592653589793), 2);
  $.indexSet(this.particles, 'p ' + $.S(this.particleNum), $.Particle$(this.mouseX, this.mouseY, $.mul($.Math_cos(dir), mag), $.mul($.Math_sin(dir), mag), 0));
 ...
 run$0: function() {
  ...
  $.window().setInterval$2(new $.Cord04_run_f(this), $.toInt(100.0));
  $.window().setInterval$2(new $.Cord04_run_f0(this), $.toInt(33.333333333333336));
 }
};

$$.Particle = {"":
 ["midPointY", "midPointX", "radius=", "ySpeed=", "xSpeed=", "y=", "x="],
 super: "Object"
};
...
$.Particle$ = function(x, y, xSpeed, ySpeed, radius) {
  return new $.Particle(null, null, radius, ySpeed, xSpeed, y, x);
};
...
$.Cord04$ = function() {
  return new $.Cord04(null, 600, 0.97, 1, 100, 25, null, null, null, null, null, null, null);
};
