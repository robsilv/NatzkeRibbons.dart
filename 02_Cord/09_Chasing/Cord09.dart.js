...
//  ********** Library C:\Users\Rob\Natzke Ribbons\02_Cord\09_Cord\Cord09 **************
// ********** Code for Cord09 **************
function Cord09() {
...
Cord09.prototype.onDocumentMouseMove = function(event) {
  this.mouseX = event.get$pageX();
  this.mouseY = event.get$pageY();
}
Cord09.prototype.get$onDocumentMouseMove = function() {
  return this.onDocumentMouseMove.bind(this);
}
Cord09.prototype.spawnParticle = function() {
  var mag = Math.random() * (2);
  var dir = Math.random() * (3.141593) * (2);
  this.particles.$setindex(("p " + this.particleNum), new Particle(this.mouseX, this.mouseY, Math.cos(dir) * mag, Math.sin(dir) * mag, (0)));
...
Cord09.prototype.moveParticles = function() {
  ...
  for (var c = this.particleNum - (1);
   c > min; c--) {
    ...
    if (Distance > this.maxDistance) {
      var oldX = p.x;
      var oldY = p.y;
      p.x = nb.x + this.maxDistance * Math.cos(Angle);
      p.y = nb.y + this.maxDistance * Math.sin(Angle);
      p.xSpeed = p.xSpeed + ((p.x - oldX) * (0.05));
      p.ySpeed = p.ySpeed + ((p.y - oldY) * (0.05));
    }
  ...
  for (var c = this.particleNum - (1);
   c > min; c--) {
    ...
    this.context.set$lineWidth(p.radius);
    ...
    this.context.set$strokeStyle("#ffffff");
    this.context.stroke();
  }
  for (var c = this.particleNum - (1);
   c > min; c--) {
