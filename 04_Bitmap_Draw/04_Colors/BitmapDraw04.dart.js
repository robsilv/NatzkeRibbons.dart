...
// ********** Code for BitmapDraw04 **************
function BitmapDraw04() {
...
BitmapDraw04.prototype.run = function() {
  var $this = this; // closure support
  this.wind = (0);
  this.mouseX = (0);
  this.mouseY = (0);
  this.particleNum = (0);
  this.particles = new HashMapImplementation();
  ...
  this.canvas.set$width(html_get$$window().get$innerWidth());
  this.canvas.set$height(html_get$$window().get$innerHeight());
  this.container.get$nodes().add(this.canvas);
  this.context = this.canvas.getContext("2d");
  this.imgCanvas = ElementWrappingImplementation.ElementWrappingImplementation$tag$factory("canvas");
  this.imgCanvas.set$width(html_get$$window().get$innerWidth());
  this.imgCanvas.set$height(html_get$$window().get$innerHeight());
  this.imgContext = this.imgCanvas.getContext("2d");
  this.image = ElementWrappingImplementation.ElementWrappingImplementation$tag$factory("img");
  this.image.set$src("Bitmap1.png");
  this.image.get$on().get$load().add$1(this.get$imageLoaded());
...
BitmapDraw04.prototype.imageLoaded = function(event) {
  var pattern = this.imgContext.createPattern(this.image, "repeat");
  this.imgContext.set$fillStyle(pattern);
  this.imgContext.fillRect((0), (0), html_get$$window().get$innerWidth(), html_get$$window().get$innerHeight());
}
BitmapDraw04.prototype.get$imageLoaded = function() {
  return this.imageLoaded.bind(this);
}
BitmapDraw04.prototype.onDocumentMouseMove = function(event) {
  this.mouseX = event.get$pageX();
  this.mouseY = event.get$pageY();
}
BitmapDraw04.prototype.get$onDocumentMouseMove = function() {
  return this.onDocumentMouseMove.bind(this);
}
BitmapDraw04.prototype.spawnParticle = function() {
  var mag = Math.random() * (2);
  var dir = Math.random() * (3.141593) * (2);
  var imgData = this.imgContext.getImageData(this.mouseX, this.mouseY, (1), (1));
  var pixels = imgData.get$data();
  var color = this.rgbToHex(pixels.$index((0)), pixels.$index((1)), pixels.$index((2)));
  this.particles.$setindex(("p " + this.particleNum), new Particle(this.mouseX, this.mouseY, Math.cos(dir) * mag, Math.sin(dir) * mag, (0), color));
...
BitmapDraw04.prototype.step = function() {
  ...
  for (var c = this.particleNum - (1);
   c > min; c--) {
    ...
    p.ySpeed = p.ySpeed * this.airFriction;
    p.x = p.x + p.xSpeed;
    p.y = p.y + p.ySpeed;
    if (c < min + (5)) {
      p.x = p.x - ((p.x - nb.x) * (0.6));
      p.y = p.y - ((p.y - nb.y) * (0.6));
    }
    if (c > this.particleNum - this.maxParticles * (0.5)) {
      p.radius = p.radius + (1.5);
    }
    else {
      p.radius = p.radius * (0.85);
    }
    if (p.y > this.ground) {
      p.y = this.ground;
      p.ySpeed = p.ySpeed * (-1);
    ...
    if (p.distance > this.maxDistance) {
      var oldX = p.x;
      var oldY = p.y;
      p.x = nb.x + this.maxDistance * Math.cos(p.angle);
      p.y = nb.y + this.maxDistance * Math.sin(p.angle);
      p.xSpeed = p.xSpeed + ((p.x - oldX) * (0.1));
      p.ySpeed = p.ySpeed + ((p.y - oldY) * (0.1));
    }
    p.midPointX = p.x + (nb.x - p.x) * (0.5);
    p.midPointY = p.y + (nb.y - p.y) * (0.5);
    p.midPointXB = p.x + (nbB.x - p.x) * (0.5);
    p.midPointYB = p.y + (nbB.y - p.y) * (0.5);
  }
}
BitmapDraw04.prototype.render = function() {
  var min = this.particleNum - this.maxParticles;
  if (min < (0)) min = (0);
  for (var c = this.particleNum - (1);
   c > min; c--) {
    ...
    if (c == this.particleNum - (1)) {
      ...
      var point_a = new Point2D(nbB.x + nbB.radius * Math.cos(nbB.angle + (1.570796)), nbB.y + nbB.radius * Math.sin(nbB.angle + (1.570796)));
      var point_b = new Point2D(nbB.x - nbB.radius * Math.cos(nbB.angle + (1.570796)), nbB.y - nbB.radius * Math.sin(nbB.angle + (1.570796)));
      var point_c = new Point2D(p.x + p.radius * Math.cos(p.angle + (1.570796)), p.y + p.radius * Math.sin(p.angle + (1.570796)));
      var point_d = new Point2D(p.x - p.radius * Math.cos(p.angle + (1.570796)), p.y - p.radius * Math.sin(p.angle + (1.570796)));
      var point_e = new Point2D(nb.x + nb.radius * Math.cos(nb.angle + (1.570796)), nb.y + nb.radius * Math.sin(nb.angle + (1.570796)));
      var point_f = new Point2D(nb.x - nb.radius * Math.cos(nb.angle + (1.570796)), nb.y - nb.radius * Math.sin(nb.angle + (1.570796)));
      var point_ac = new Point2D(point_a.x + (point_c.x - point_a.x) * (0.5), point_a.y + (point_c.y - point_a.y) * (0.5));
      var point_ce = new Point2D(point_c.x + (point_e.x - point_c.x) * (0.5), point_c.y + (point_e.y - point_c.y) * (0.5));
      var point_bd = new Point2D(point_b.x + (point_d.x - point_b.x) * (0.5), point_b.y + (point_d.y - point_b.y) * (0.5));
      var point_df = new Point2D(point_d.x + (point_f.x - point_d.x) * (0.5), point_d.y + (point_f.y - point_d.y) * (0.5));
      var rgb = this.hexToRGB(p.color);
      this.context.beginPath();
      this.context.set$fillStyle(("rgba( " + rgb.$index((0)) + ", " + rgb.$index((1)) + ", " + rgb.$index((2)) + ", .25 )"));
      this.context.moveTo(point_bd.x, point_bd.y);
      this.context.quadraticCurveTo(point_d.x, point_d.y, point_df.x, point_df.y);
      this.context.lineTo(point_ce.x, point_ce.y);
      this.context.quadraticCurveTo(point_c.x, point_c.y, point_ac.x, point_ac.y);
      this.context.lineTo(point_bd.x, point_bd.y);
      this.context.fill();
      rgb = this.hexToRGB((2236962));
      this.context.set$lineWidth((1));
      this.context.set$lineJoin("round");
      this.context.set$lineCap("round");
      this.context.set$strokeStyle(("rgba( " + rgb.$index((0)) + ", " + rgb.$index((1)) + ", " + rgb.$index((2)) + ", 0 )"));
      this.context.stroke();
      this.context.beginPath();
      this.context.moveTo(point_bd.x, point_bd.y);
      this.context.quadraticCurveTo(point_d.x, point_d.y, point_df.x, point_df.y);
      this.context.moveTo(point_ce.x, point_ce.y);
      this.context.quadraticCurveTo(point_c.x, point_c.y, point_ac.x, point_ac.y);
      rgb = this.hexToRGB((2236962));
      this.context.set$lineWidth((1));
      this.context.set$lineJoin("round");
      this.context.set$lineCap("round");
      this.context.set$strokeStyle(("rgba( " + rgb.$index((0)) + ", " + rgb.$index((1)) + ", " + rgb.$index((2)) + ", 1 )"));
      this.context.stroke();
    }
  }
}
BitmapDraw04.prototype.hexToRGB = function(hex) {
  hex = hex.floor();
  var r = (hex >> (16) & (255));
  var g = (hex >> (8) & (255));
  var b = (hex & (255));
...
BitmapDraw04.prototype.rgbToHex = function(r, g, b) {
  var h = (r << (16)) ^ (g << (8)) ^ (b);
  return h;
}
// ********** Code for Particle **************
...
(function(){
  ...
  var v8/*Entry*/ = 'Entry|DirectoryEntry|FileEntry';
  ...
  var v11/*EventTarget*/ = [v3/*AbstractWorker*/,v4/*Node*/,'EventTarget|DOMApplicationCache|DOMWindow|EventSource|MessagePort|Notification|SVGElementInstance|WebSocket|XMLHttpRequest|XMLHttpRequestUpload'].join('|');
