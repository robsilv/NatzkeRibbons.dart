...
Ribbon03.prototype.run = function() {
  var $this = this; // closure support
  this.wind = (0);
  this.mouseX = (0);
  this.mouseY = (0);
...
Ribbon03.prototype.onDocumentMouseMove = function(event) {
  this.mouseX = event.get$pageX();
  this.mouseY = event.get$pageY();
}
Ribbon03.prototype.get$onDocumentMouseMove = function() {
  return this.onDocumentMouseMove.bind(this);
}
Ribbon03.prototype.spawnParticle = function() {
...
Ribbon03.prototype.moveParticles = function() {
  ...
  for (var c = this.particleNum - (1);
   c > min; c--) {
    ...
    if (c == this.particleNum - (1)) {
      ...
      this.context.moveTo(p.x + (20) * Math.cos(p.angle), p.y + (20) * Math.sin(p.angle));
      this.context.lineTo(p.x - (20) * Math.cos(p.angle), p.y - (20) * Math.sin(p.angle));
    }
    this.context.set$lineJoin("round");
    this.context.set$lineCap("round");
